...
This update mainly improves the organization of the repository and adds the files required for model evaluation.

Specifically, the project structure has been reorganized by renaming the original `scripts/` directory to `src/` and moving `biological_database/` into the `data/` directory. In addition, the evaluation code has been updated, and the evaluation notebook `eval.ipynb` has been added. The `top50_gene` files used for evaluation are also included in this update.

It should also be noted that the cell segmentation results for the DLPFC dataset used in this study were not generated by HoVer-Net. Instead, they were obtained using the official MATLAB implementation. To facilitate reproducibility, we have organized and uploaded the corresponding segmentation results for the DLPFC dataset, which can be downloaded from the following link:

File: `DLPFC_segmentation_results.tar.gz`
Link: `https://pan.baidu.com/s/1BZxbqOunGY6K6O28tqgD2w?pwd=ufy9`
Extraction code: `ufy9`

For the kidney cancer and breast cancer datasets, cell segmentation can be performed following the instructions provided in the repository README.
